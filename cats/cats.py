...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_para = [para for para in paragraphs if select(para)]
    para_num = len(select_para)
    if k >= para_num:
        return ''
    else:
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(para):
        content = split(remove_punctuation(lower(para)))
        for i in content:
            if i in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    correct = 0
    typeall, full = len(typed_words), len(source_words)
    least = min(full, typeall)

    for i in range(0, least):
        if typed_words[i] == source_words[i]:
            correct += 1

    if typeall == 0 and full == 0:
        return 100.0
    if typeall == 0:
        return 0.0
    return correct / typeall * 100
    # END PROBLEM 3
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    upload({'id': user_id, 'progress': correct / len(source)})
