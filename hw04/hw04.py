...
def balanced(m):
    ...
    if is_planet(m):
        ...
        len_left, len_right = length(left(m)), length(right(m))
        end_left, end_right = end(left(m)), end(right(m))
        torque_left, torque_right = len_left * total_mass(end_left), len_right * total_mass(end_right)
...
def berry_finder(t):
    ...
    if is_leaf(t):
        ...
        return any([berry_finder(bs) for bs in branches(t)])
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        return max([max_path_sum(bs) for bs in branches(t)]) + label(t)
